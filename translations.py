...
TRANSLATIONS = {
    "tr": {
        ...
        "app_subtitle": "PDF'den Rusça kelimeleri çıkar, temiz bir CSV veya Word dosyası olarak dışa aktar.",
        "language": "Dil",
        ...
        "msg_cancel_before_switch": "Farklı bir PDF seçmeden önce mevcut işlemi iptal et.",
        "msg_pdf_queued": "İptal tamamlanınca yeni PDF yüklenecek: {name}",
        "msg_output_locked": "Çıktı dosyası başka bir programda açık görünüyor. Lütfen dosyayı kapat veya farklı bir kayıt yolu seç, sonra tekrar dene.\n\nDosya: {path}",
        "msg_output_perm_denied": "Bu konuma yazma izni yok. Lütfen farklı bir kayıt yolu seç.\n\nDosya: {path}",
        "msg_output_write_failed": "Çıktı dosyası yazılamadı: {err}\n\nDosya: {path}",
        "msg_output_unsupported": "Desteklenmeyen çıktı formatı. Lütfen .csv veya .docx seç.\n\nDosya: {path}",
        "msg_docx_missing": "Word çıktısı için 'python-docx' kurulu değil. Kurmak için: pip install python-docx",

        "dlg_pdf_title": "PDF Seç",
        "dlg_output_title": "Çıktıyı nereye kaydedeyim?",
        "dlg_pdf_filter": "PDF dosyaları (*.pdf)",
        "dlg_output_filter": "Desteklenen dosyalar (*.csv *.docx);;CSV dosyaları (*.csv);;Word dosyaları (*.docx)",
        "dlg_output_csv_filter": "CSV dosyaları (*.csv)",
        "dlg_output_docx_filter": "Word dosyaları (*.docx)",
        "dlg_exclude_title": "Hariç tutulacak kelimeler (.txt)",
        "dlg_txt_filter": "Metin dosyaları (*.txt)",

        "export_mode": "Dışa aktarma",
        "export_single": "Tek dosya",
        "export_separate": "Türlere ayır (her POS ayrı dosya)",
        "export_both": "Her ikisi",
        "export_hint": "Yukarıdaki “Kelime Türleri” pillerinden sadece istediklerini seçebilirsin (ör. yalnız fiil). Kayıt yolunda .csv veya .docx seçebilirsin. “Türlere ayır” seçilirse seçtiğin her tür ayrı bir dosya olur.",
        "output_placeholder": "…/kelimeler.csv veya .docx",
    ...
    "ru": {
        ...
        "app_subtitle": "Извлеките русские слова из PDF и экспортируйте в CSV или Word.",
        "language": "Язык",
        ...
        "msg_cancel_before_switch": "Сначала отмените текущую обработку, затем выберите другой PDF.",
        "msg_pdf_queued": "Как только отмена завершится, будет загружен новый PDF: {name}",
        "msg_output_locked": "Похоже, выходной файл открыт в другой программе. Закройте файл или выберите другой путь сохранения и попробуйте снова.\n\nФайл: {path}",
        "msg_output_perm_denied": "Нет прав на запись в это место. Выберите другой путь сохранения.\n\nФайл: {path}",
        "msg_output_write_failed": "Не удалось записать выходной файл: {err}\n\nФайл: {path}",
        "msg_output_unsupported": "Неподдерживаемый формат вывода. Выберите .csv или .docx.\n\nФайл: {path}",
        "msg_docx_missing": "Для экспорта в Word требуется 'python-docx'. Установите: pip install python-docx",

        "dlg_pdf_title": "Выбрать PDF",
        "dlg_output_title": "Куда сохранить файл?",
        "dlg_pdf_filter": "PDF файлы (*.pdf)",
        "dlg_output_filter": "Поддерживаемые файлы (*.csv *.docx);;CSV файлы (*.csv);;Word файлы (*.docx)",
        "dlg_output_csv_filter": "CSV файлы (*.csv)",
        "dlg_output_docx_filter": "Word файлы (*.docx)",
        "dlg_exclude_title": "Файл исключений (.txt)",
        "dlg_txt_filter": "Текстовые файлы (*.txt)",

        "export_mode": "Режим экспорта",
        "export_single": "Один файл",
        "export_separate": "По частям речи (каждая в своём файле)",
        "export_both": "Оба",
        "export_hint": "Над этой картой включи только нужные «Части речи» (например, только глаголы). В пути сохранения можно выбрать .csv или .docx. При выборе «По частям речи» каждая включённая часть сохранится в отдельный файл.",
        "output_placeholder": "…/slova.csv или .docx",
    ...
    "en": {
        ...
        "app_subtitle": "Extract Russian words from a PDF and export a clean CSV or Word file.",
        "language": "Language",
        ...
        "msg_cancel_before_switch": "Cancel the current extraction before switching to another PDF.",
        "msg_pdf_queued": "The new PDF will load as soon as cancellation finishes: {name}",
        "msg_output_locked": "The output file appears to be open in another program. Please close it or pick a different save path and try again.\n\nFile: {path}",
        "msg_output_perm_denied": "No permission to write to that location. Please pick a different save path.\n\nFile: {path}",
        "msg_output_write_failed": "Could not write the output file: {err}\n\nFile: {path}",
        "msg_output_unsupported": "Unsupported output format. Please choose a .csv or .docx file.\n\nFile: {path}",
        "msg_docx_missing": "'python-docx' is required for Word export. Install it with: pip install python-docx",

        "dlg_pdf_title": "Select a PDF",
        "dlg_output_title": "Save output as…",
        "dlg_pdf_filter": "PDF files (*.pdf)",
        "dlg_output_filter": "Supported files (*.csv *.docx);;CSV files (*.csv);;Word documents (*.docx)",
        "dlg_output_csv_filter": "CSV files (*.csv)",
        "dlg_output_docx_filter": "Word documents (*.docx)",
        "dlg_exclude_title": "Words to exclude (.txt)",
        "dlg_txt_filter": "Text files (*.txt)",

        "export_mode": "Export mode",
        "export_single": "Single file",
        "export_separate": "Split by POS (one file each)",
        "export_both": "Both",
        "export_hint": "Use the “Parts of Speech” pills above to keep only what you want (e.g. verbs only). Choose .csv or .docx in the save path. With “Split by POS”, each enabled part of speech is written to its own file.",
        "output_placeholder": "…/words.csv or .docx",
